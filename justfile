...
#
# 8. scripts/score.py mirrors the check recipes' commands in machine-readable
#    mode. When changing any code-* or test recipe's tool, flags, or paths,
#    update scripts/score.py to match (and vice versa).
# =============================================================================
...
help:
    ...
    @printf "  %-40s %s\n" "benchmark" "Run the full jevals suite on every model, then print the results table"
    @printf "  %-40s %s\n" "report" "Print the results tables and write reports/benchmark/report.md"
    @echo ""
    ...
    @printf "  %-40s %s\n" "score" "Compute the codebase quality score"
    @echo ""
...
init: check
    ...
    @uv run pre-commit install \
        || { printf "\033[31m✗ init failed: could not install the pre-commit hook\033[0m\n"; exit 1; }
...
# Answer the example question with every model and both methods
run:
    @echo ""
...
# and check every item's label against its source row
fetch:
    @echo ""
...
# Run the full jevals suite on every model (resumes where it stopped), then print the results table
benchmark: fetch
    @echo ""
...
# Print the results tables from the finished runs and write reports/benchmark/report.md
report:
    @echo ""
...
code-semgrep:
    ...
    @uv run semgrep --config config/semgrep/ --error src scripts tests pyproject.toml
    @echo ""
    @printf "\033[0;32m✓ Semgrep checks passed\033[0m\n"
    @echo ""

# Scan dependencies for known vulnerabilities
# Ignored advisories (unfixable: every semgrep release pins click~=8.1.8 and
# mcp==1.23.3, while fixes only exist in click 8.3.3 / mcp 1.27.2+ — remove
# these ignores as soon as semgrep relaxes its pins):
#   PYSEC-2026-2132  click: command injection via click.edit()
#   CVE-2026-52870   mcp: cross-session task visibility
#   CVE-2026-52869   mcp: session takeover via session id
...
# Compute the deterministic codebase quality score
score:
    @echo ""
    @printf "\033[0;34m=== Computing Codebase Quality Score ===\033[0m\n"
    @mkdir -p reports/score
    @uv run python scripts/score.py
    @printf "\033[0;32m✓ Score report written to reports/score/score.json\033[0m\n"
    @echo ""
