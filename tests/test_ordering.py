"""The shuffle must match the JavaScript reference in the jevals-data README bit for bit."""
...
def test_matches_javascript_reference_for_banking77_item() -> None:
    # Expected values produced by running the README's JavaScript in Node.
    order = option_order("banking77-2", BANKING77_OPTIONS, 0)
    assert order[:6] == [
        "pending_transfer",
    ...
    assert sorted(order) == sorted(BANKING77_OPTIONS)
...
def test_mulberry32_is_deterministic_and_in_unit_interval() -> None:
    first, second = mulberry32(42), mulberry32(42)
    values = [first() for _ in range(100)]
    assert values == [second() for _ in range(100)]
    assert all(0 <= value < 1 for value in values)


def test_different_seeds_give_different_orders() -> None:
    assert option_order("banking77-2", BANKING77_OPTIONS, 0) != option_order("banking77-2", BANKING77_OPTIONS, 1)


@pytest.mark.parametrize(("epoch", "seed"), [(0, 0), (1, 0), (2, 1), (3, 2), (4, 3)])
def test_order_seed_per_repeat(epoch: int, seed: int) -> None:
    assert order_seed(epoch) == seed


def test_order_seed_rejects_negative_epoch() -> None:
